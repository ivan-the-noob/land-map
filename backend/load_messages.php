...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['property_id'], $_POST['agent_id'])) {
    ...
    // List of blocked words (allows detection even if spaces are removed)
    $blocked_words = [
    ...
    for ($i = 0; $i < $totalMessages; $i++) {
        ...
        echo '<div class="chat-message d-flex ' . $isUser . '" style="gap: 5px; align-items: flex-start;">' . 
                ...
                '<div class="message-time" style="font-size: 12px; color: grey; margin-top: 2px;">' . $messageTime . '</div>' . 
                '</div>
              </div>';
    }
}
